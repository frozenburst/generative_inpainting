...
def toAudio_2amp_denorm(db_spec_norm):
    h, w, _ = db_spec_norm.shape
    db_spec_norm = db_spec_norm.reshape((1, h, w))
    db_spec_norm = torch.from_numpy(db_spec_norm)
    ...
    c, t = re_waveform.shape
    res_waveform = re_waveform.reshape([t, c])
    return res_waveform
